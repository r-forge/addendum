...
#' @usage \method{crossValidate}{EMLasso.1l.lognet}(model, ds=model$dfr, out=model$resp, wts=rep(1, nrow(ds)), dsconvprobs, needPredict=0, betweenColAndLevel="",..., verbosity=0)
	...
	keepResultPerLambda=FALSE, verbosity=0)
{
	...
	lambda<-model$lambda
	cvm<-try(sapply(cvlogreglist, "[[", "cvm"))
	cvsd<-try(sapply(cvlogreglist, "[[", "cvsd"))
